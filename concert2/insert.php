...
<?
	...
	*/
	...
	if ($mode=="modify")
	{
		...
		$sql = "update $table set subject='$subject', about='$about', content='$content' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
		...
		$sql = "insert into $table (id, name, about, nick, subject, content, regist_day, hit, is_html, ";
		$sql .= " file_name_0, file_name_1, file_name_2, file_copied_0,  file_copied_1, file_copied_2) ";
		$sql .= "values('$userid', '$username', '$usernick', '$subject','$about' '$content', '$regist_day', 0, '$is_html', ";
